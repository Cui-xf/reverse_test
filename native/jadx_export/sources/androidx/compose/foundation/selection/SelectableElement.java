...
final class SelectableElement extends ModifierNodeElement<SelectableNode> {
    ...
    public void update(SelectableNode node) {
        node.m943updateQzZPfjk(this.selected, this.interactionSource, this.indicationNodeFactory, this.enabled, this.role, this.onClick);
    }
    ...
    @Override // androidx.compose.ui.node.ModifierNodeElement
    public int hashCode() {
        int result = ChangeSize$$ExternalSyntheticBackport0.m(this.selected);
        int i = result * 31;
        ...
        Role role = this.role;
        return ((result4 + (role != null ? Role.m5953hashCodeimpl(role.getValue()) : 0)) * 31) + this.onClick.hashCode();
    }
}
