...
public final class FlowLayoutBuildingBlocks {
    ...
    public final WrapEllipsisInfo getWrapEllipsisInfo(WrapInfo wrapInfo, boolean hasNext, int lastContentLineIndex, int totalCrossAxisSize, int leftOverMainAxis, int nextIndexInLine) {
        ...
        boolean canFitLine = lastContentLineIndex >= 0 && (nextIndexInLine == 0 || (leftOverMainAxis - IntIntPair.m24getFirstimpl(ellipsisInfo.getEllipsisSize()) >= 0 && nextIndexInLine < this.maxItemsInMainAxis));
        ellipsisInfo.setPlaceEllipsisOnLastContentLine(canFitLine);
        return ellipsisInfo;
    }

    /* renamed from: getWrapInfo-OpUlnko, reason: not valid java name */
    public final WrapInfo m603getWrapInfoOpUlnko(boolean nextItemHasNext, int nextIndexInLine, long leftOver, IntIntPair nextSize, int lineIndex, int totalCrossAxisSize, int currentLineCrossAxisSize, boolean isWrappingRound, boolean isEllipsisWrap) {
        boolean z;
        boolean z2;
        IntIntPair ellipsis;
        boolean z3;
        ...
        if (this.overflow.getType$foundation_layout_release() == FlowLayoutOverflow.OverflowType.Visible) {
            ...
            z = lineIndex >= this.maxLines || IntIntPair.m25getSecondimpl(leftOver) - IntIntPair.m25getSecondimpl(nextSize.getPackedValue()) < 0;
        }
        ...
        if (nextIndexInLine == 0) {
            ...
            z2 = nextIndexInLine >= this.maxItemsInMainAxis || IntIntPair.m24getFirstimpl(leftOver) - IntIntPair.m24getFirstimpl(nextSize.getPackedValue()) < 0;
        }
        ...
        if (shouldWrapItem) {
            ...
            return new WrapInfo(true, m603getWrapInfoOpUlnko(nextItemHasNext, 0, IntIntPair.m20constructorimpl(Constraints.m6636getMaxWidthimpl(arg0$iv), (IntIntPair.m25getSecondimpl(leftOver) - this.crossAxisSpacing) - currentLineCrossAxisSize), IntIntPair.m17boximpl(IntIntPair.m20constructorimpl(IntIntPair.m24getFirstimpl(nextSize.getPackedValue()) - this.mainAxisSpacing, IntIntPair.m25getSecondimpl(nextSize.getPackedValue()))), lineIndex + 1, totalContainerCrossAxisSize, 0, true, false).getIsLastItemInContainer());
        }
        int totalContainerCrossAxisSize2 = totalCrossAxisSize + Math.max(currentLineCrossAxisSize, IntIntPair.m25getSecondimpl(nextSize.getPackedValue()));
        if (isEllipsisWrap) {
            ellipsis = null;
        } else {
            ellipsis = this.overflow.m608ellipsisSizeF35zmw$foundation_layout_release(nextItemHasNext, lineIndex, totalContainerCrossAxisSize2);
        }
        if (ellipsis != null) {
            ellipsis.getPackedValue();
            z3 = nextIndexInLine + 1 >= this.maxItemsInMainAxis || ((IntIntPair.m24getFirstimpl(leftOver) - IntIntPair.m24getFirstimpl(nextSize.getPackedValue())) - this.mainAxisSpacing) - IntIntPair.m24getFirstimpl(ellipsis.getPackedValue()) < 0;
        } else {
            z3 = false;
        }
        ...
        if (shouldWrapEllipsis) {
            ...
            WrapInfo wrapInfo = m603getWrapInfoOpUlnko(false, 0, IntIntPair.m20constructorimpl(Constraints.m6636getMaxWidthimpl(arg0$iv2), (IntIntPair.m25getSecondimpl(leftOver) - this.crossAxisSpacing) - Math.max(currentLineCrossAxisSize, IntIntPair.m25getSecondimpl(nextSize.getPackedValue()))), ellipsis, lineIndex + 1, totalContainerCrossAxisSize2, 0, true, true);
