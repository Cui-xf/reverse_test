...
public final class TextFieldDelegate {
    ...
    public static final class Companion {
        ...
        public static /* synthetic */ Triple m1071layout_EkL_Y$foundation_release$default(Companion companion, TextDelegate textDelegate, long j, LayoutDirection layoutDirection, TextLayoutResult textLayoutResult, int i, Object obj) {
            ...
            return companion.m1074layout_EkL_Y$foundation_release(textDelegate, j, layoutDirection, textLayoutResult2);
        }

        @JvmStatic
        /* renamed from: layout-_EkL_-Y$foundation_release, reason: not valid java name */
        public final Triple<Integer, Integer, TextLayoutResult> m1074layout_EkL_Y$foundation_release(TextDelegate textDelegate, long constraints, LayoutDirection layoutDirection, TextLayoutResult prevResultText) {
            TextLayoutResult layoutResult = textDelegate.m1061layoutNN6EwU(constraints, layoutDirection, prevResultText);
            return new Triple<>(Integer.valueOf(IntSize.m6867getWidthimpl(layoutResult.getSize())), Integer.valueOf(IntSize.m6866getHeightimpl(layoutResult.getSize())), layoutResult);
        }

        @JvmStatic
        /* renamed from: draw-Q1vqE60$foundation_release, reason: not valid java name */
        public final void m1073drawQ1vqE60$foundation_release(Canvas canvas, TextFieldValue value, long selectionPreviewHighlightRange, long deletionPreviewHighlightRange, OffsetMapping offsetMapping, TextLayoutResult textLayoutResult, Paint highlightPaint, long selectionBackgroundColor) {
            if (!TextRange.m6140getCollapsedimpl(selectionPreviewHighlightRange)) {
                highlightPaint.mo4066setColor8_81llA(selectionBackgroundColor);
                m1070drawHighlightLepunE(canvas, selectionPreviewHighlightRange, offsetMapping, textLayoutResult, highlightPaint);
            } else if (!TextRange.m6140getCollapsedimpl(deletionPreviewHighlightRange)) {
                Color colorM4177boximpl = Color.m4177boximpl(textLayoutResult.getLayoutInput().getStyle().m6176getColor0d7_KjU());
                long it = colorM4177boximpl.m4197unboximpl();
                if (it == 16) {
                    colorM4177boximpl = null;
                }
                long textColor = colorM4177boximpl != null ? colorM4177boximpl.m4197unboximpl() : Color.INSTANCE.m4213getBlack0d7_KjU();
                highlightPaint.mo4066setColor8_81llA(Color.m4185copywmQWz5c(textColor, (14 & 1) != 0 ? Color.m4189getAlphaimpl(textColor) : Color.m4189getAlphaimpl(textColor) * 0.2f, (14 & 2) != 0 ? Color.m4193getRedimpl(textColor) : 0.0f, (14 & 4) != 0 ? Color.m4192getGreenimpl(textColor) : 0.0f, (14 & 8) != 0 ? Color.m4190getBlueimpl(textColor) : 0.0f));
                m1070drawHighlightLepunE(canvas, deletionPreviewHighlightRange, offsetMapping, textLayoutResult, highlightPaint);
            } else if (!TextRange.m6140getCollapsedimpl(value.getSelection())) {
                highlightPaint.mo4066setColor8_81llA(selectionBackgroundColor);
                m1070drawHighlightLepunE(canvas, value.getSelection(), offsetMapping, textLayoutResult, highlightPaint);
            }
            TextPainter.INSTANCE.paint(canvas, textLayoutResult);
        }

        /* renamed from: drawHighlight-Le-punE, reason: not valid java name */
        private final void m1070drawHighlightLepunE(Canvas canvas, long range, OffsetMapping offsetMapping, TextLayoutResult textLayoutResult, Paint paint) {
            int start = offsetMapping.originalToTransformed(TextRange.m6144getMinimpl(range));
            int end = offsetMapping.originalToTransformed(TextRange.m6143getMaximpl(range));
            if (start != end) {
        ...
        public final void notifyFocusedRect$foundation_release(TextFieldValue value, TextDelegate textDelegate, TextLayoutResult textLayoutResult, LayoutCoordinates layoutCoordinates, TextInputSession textInputSession, boolean hasFocus, OffsetMapping offsetMapping) {
            ...
            int focusOffsetInTransformed = offsetMapping.originalToTransformed(TextRange.m6143getMaximpl(value.getSelection()));
            if (focusOffsetInTransformed < textLayoutResult.getLayoutInput().getText().length()) {
                ...
                bbox = new Rect(0.0f, 0.0f, 1.0f, IntSize.m6866getHeightimpl(defaultSize));
            }
            long globalLT = layoutCoordinates.mo5540localToRootMKHz9U(OffsetKt.Offset(bbox.getLeft(), bbox.getTop()));
            textInputSession.notifyFocusedRect(RectKt.m3985Recttz77jQw(OffsetKt.Offset(Offset.m3945getXimpl(globalLT), Offset.m3946getYimpl(globalLT)), SizeKt.Size(bbox.getWidth(), bbox.getHeight())));
        }
            ...
            if (innerTextFieldCoordinates != null && innerTextFieldCoordinates.isAttached() && (decorationBoxCoordinates = textLayoutResult.getDecorationBoxCoordinates()) != null) {
                textInputSession.updateTextLayoutResult(textFieldValue, offsetMapping, textLayoutResult.getValue(), new Function1<Matrix, Unit>() { // from class: androidx.compose.foundation.text.TextFieldDelegate$Companion$updateTextLayoutResult$1$1$1
                    ...
                    public /* bridge */ /* synthetic */ Unit invoke(Matrix matrix) {
                        m1076invoke58bKbWc(matrix.m4445unboximpl());
                        return Unit.INSTANCE;
                    }

                    /* renamed from: invoke-58bKbWc, reason: not valid java name */
                    public final void m1076invoke58bKbWc(float[] matrix) {
                        if (innerTextFieldCoordinates.isAttached()) {
                            LayoutCoordinatesKt.findRootCoordinates(innerTextFieldCoordinates).mo5544transformFromEL8BTi8(innerTextFieldCoordinates, matrix);
                        }
                    }
        ...
        public final void m1075setCursorOffsetULxng0E$foundation_release(long position, TextLayoutResultProxy textLayoutResult, EditProcessor editProcessor, OffsetMapping offsetMapping, Function1<? super TextFieldValue, Unit> onValueChange) {
            int offset = offsetMapping.transformedToOriginal(TextLayoutResultProxy.m1096getOffsetForPosition3MmeM6k$default(textLayoutResult, position, false, 2, null));
            onValueChange.invoke(TextFieldValue.m6390copy3r_uNRQ$default(editProcessor.toTextFieldValue(), (AnnotatedString) null, TextRangeKt.TextRange(offset), (TextRange) null, 5, (Object) null));
        }
        ...
        public final void onBlur$foundation_release(TextInputSession textInputSession, EditProcessor editProcessor, Function1<? super TextFieldValue, Unit> onValueChange) {
            onValueChange.invoke(TextFieldValue.m6390copy3r_uNRQ$default(editProcessor.toTextFieldValue(), (AnnotatedString) null, 0L, (TextRange) null, 3, (Object) null));
            textInputSession.dispose();
        }

        /* renamed from: applyCompositionDecoration-72CqOWE, reason: not valid java name */
        public final TransformedText m1072applyCompositionDecoration72CqOWE(long compositionRange, TransformedText transformed) {
            int startPositionTransformed = transformed.getOffsetMapping().originalToTransformed(TextRange.m6146getStartimpl(compositionRange));
            int endPositionTransformed = transformed.getOffsetMapping().originalToTransformed(TextRange.m6141getEndimpl(compositionRange));
