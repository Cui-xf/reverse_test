...
public final class SelectionManagerKt {
        ...
        while (index$iv2 < size) {
            ...
            if (startOffset == endOffset) {
                ...
                if (localCoordinates == null) {
                    ...
                    long containerTopLeft = layoutCoordinates.mo5538localPositionOfR5De75A(localCoordinates, localTopLeft);
                    long containerBottomRight = layoutCoordinates.mo5538localPositionOfR5De75A(localCoordinates, localBottomRight);
                    float containerLeft = Math.min(containerTop, Offset.m3945getXimpl(containerTopLeft));
                    float containerLeft2 = Offset.m3946getYimpl(containerTopLeft);
                    float containerTop2 = Math.min(containerRight, containerLeft2);
                    float containerTop3 = Offset.m3945getXimpl(containerBottomRight);
                    float containerRight3 = Math.max(containerRight2, containerTop3);
                    float containerRight4 = Offset.m3946getYimpl(containerBottomRight);
    ...
    private static final long m1382getMagnifierCenterJVtK1S4(SelectionManager manager, long magnifierSize, Selection.AnchorInfo anchor) {
        ...
        if (selectable != null && (containerCoordinates = manager.getContainerLayoutCoordinates()) != null && (selectableCoordinates = selectable.getLayoutCoordinates()) != null && (offset = anchor.getOffset()) <= selectable.getLastVisibleOffset()) {
            ...
            long localDragPosition = selectableCoordinates.mo5538localPositionOfR5De75A(containerCoordinates, offsetM1369getCurrentDragPosition_m7T9E.getPackedValue());
            float dragX = Offset.m3945getXimpl(localDragPosition);
            long lineRange = selectable.mo1331getRangeOfLineContainingjx7JFs(offset);
            if (TextRange.m6140getCollapsedimpl(lineRange)) {
                lineStartX = selectable.getLineLeft(offset);
            } else {
                float lineStartX2 = selectable.getLineLeft(TextRange.m6146getStartimpl(lineRange));
                float lineEndX = selectable.getLineRight(TextRange.m6141getEndimpl(lineRange) - 1);
            ...
            if (!IntSize.m6865equalsimpl0(magnifierSize, IntSize.INSTANCE.m6872getZeroYbymL2g()) && Math.abs(dragX - lineStartX) > IntSize.m6867getWidthimpl(magnifierSize) / 2) {
                return Offset.INSTANCE.m3960getUnspecifiedF1C5BW0();
            }
            float lineCenterY = selectable.getCenterYForOffset(offset);
            return (lineCenterY > (-1.0f) ? 1 : (lineCenterY == (-1.0f) ? 0 : -1)) == 0 ? Offset.INSTANCE.m3960getUnspecifiedF1C5BW0() : containerCoordinates.mo5538localPositionOfR5De75A(selectableCoordinates, OffsetKt.Offset(lineStartX, lineCenterY));
        }
        return Offset.INSTANCE.m3960getUnspecifiedF1C5BW0();
    }
    ...
    public static final Rect visibleBounds(LayoutCoordinates $this$visibleBounds) {
        ...
        return RectKt.m3983Rect0a9Yr6o($this$visibleBounds.mo5546windowToLocalMKHz9U(boundsInWindow.m3980getTopLeftF1C5BW0()), $this$visibleBounds.mo5546windowToLocalMKHz9U(boundsInWindow.m3974getBottomRightF1C5BW0()));
    }
