...
final class DefaultPagerNestedScrollConnection implements NestedScrollConnection {
    ...
    public final long m920consumeOnOrientationQWom1Mo(long $this$consumeOnOrientation_u2dQWom1Mo, Orientation orientation) {
        if (orientation == Orientation.Vertical) {
            return Velocity.m6930copyOhffZ5M$default($this$consumeOnOrientation_u2dQWom1Mo, 0.0f, 0.0f, 2, null);
        }
        return Velocity.m6930copyOhffZ5M$default($this$consumeOnOrientation_u2dQWom1Mo, 0.0f, 0.0f, 1, null);
    }
    ...
    public long mo479onPreScrollOzD1aCk(long available, int source) {
        float minBound;
        float maxBound;
        if (NestedScrollSource.m5286equalsimpl0(source, NestedScrollSource.INSTANCE.m5298getUserInputWNlRxjI()) && Math.abs(this.state.getCurrentPageOffsetFraction()) > 1.0E-6d) {
            ...
            float delta = this.orientation == Orientation.Horizontal ? Offset.m3945getXimpl(available) : Offset.m3946getYimpl(available);
            float coerced = RangesKt.coerceIn(delta, minBound, maxBound);
            float consumed = -this.state.dispatchRawDelta(-coerced);
            return Offset.m3938copydBAh8RU(available, this.orientation == Orientation.Horizontal ? consumed : Offset.m3945getXimpl(available), this.orientation == Orientation.Vertical ? consumed : Offset.m3946getYimpl(available));
        }
    ...
    public long mo477onPostScrollDzOQY0M(long consumed, long available, int source) {
        if (NestedScrollSource.m5286equalsimpl0(source, NestedScrollSource.INSTANCE.m5297getSideEffectWNlRxjI())) {
            if (!(m919mainAxisk4lQ0M(available) == 0.0f)) {
    ...
    public Object mo476onPostFlingRZ2iAVY(long consumed, long available, Continuation<? super Velocity> continuation) {
        return Velocity.m6925boximpl(m920consumeOnOrientationQWom1Mo(available, this.orientation));
    }

    /* renamed from: mainAxis-k-4lQ0M, reason: not valid java name */
    private final float m919mainAxisk4lQ0M(long $this$mainAxis_u2dk_u2d4lQ0M) {
