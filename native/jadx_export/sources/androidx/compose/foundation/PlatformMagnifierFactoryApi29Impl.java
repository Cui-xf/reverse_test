...
public final class PlatformMagnifierFactoryApi29Impl implements PlatformMagnifierFactory {
    ...
    public PlatformMagnifierImpl mo321createnHHXs2Y(View view, boolean useTextDefault, long size, float cornerRadius, float elevation, boolean clippingEnabled, Density density, float initialZoom) {
        if (useTextDefault) {
            return new PlatformMagnifierImpl(new Magnifier(view));
        }
        long pixelSize = density.mo368toSizeXkaWNTQ(size);
        float pixelCornerRadius = density.mo367toPx0680j_4(cornerRadius);
        float pixelElevation = density.mo367toPx0680j_4(elevation);
        ...
        if (pixelSize != InlineClassHelperKt.UnspecifiedPackedFloats) {
            $this$create_nHHXs2Y_u24lambda_u241_u24lambda_u240.setSize(MathKt.roundToInt(Size.m4014getWidthimpl(pixelSize)), MathKt.roundToInt(Size.m4011getHeightimpl(pixelSize)));
        }
    ...
    public static final class PlatformMagnifierImpl extends PlatformMagnifierFactoryApi28Impl.PlatformMagnifierImpl {
        ...
        public void mo320updateWko1d7g(long sourceCenter, long magnifierCenter, float zoom) {
            if (!Float.isNaN(zoom)) {
                getMagnifier().setZoom(zoom);
            }
            if (OffsetKt.m3964isSpecifiedk4lQ0M(magnifierCenter)) {
                getMagnifier().show(Offset.m3945getXimpl(sourceCenter), Offset.m3946getYimpl(sourceCenter), Offset.m3945getXimpl(magnifierCenter), Offset.m3946getYimpl(magnifierCenter));
            } else {
                getMagnifier().show(Offset.m3945getXimpl(sourceCenter), Offset.m3946getYimpl(sourceCenter));
            }
        }
    }
}
