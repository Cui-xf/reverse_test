...
public final class RangeSliderState {
    ...
    public final void onDrag$material3_release(boolean isStart, float offset) {
        ...
        long scaledUserValue = m2413scaleToUserValueowVgs5E(getMinPx(), getMaxPx(), offsetRange);
        if (!SliderRange.m2535equalsimpl0(scaledUserValue, SliderKt.SliderRange(getActiveRangeStart(), getActiveRangeEnd()))) {
            if (this.onValueChange != null) {
                Function1 it = this.onValueChange;
                if (it != null) {
                    it.invoke(SliderRange.m2532boximpl(scaledUserValue));
                    return;
                }
                return;
            }
            setActiveRangeStart(SliderRange.m2537getStartimpl(scaledUserValue));
            setActiveRangeEnd(SliderRange.m2536getEndInclusiveimpl(scaledUserValue));
        }
    }
    ...
    private final long m2413scaleToUserValueowVgs5E(float minPx, float maxPx, long offset) {
        return SliderKt.m2525scaleziovWd0(minPx, maxPx, offset, this.valueRange.getStart().floatValue(), this.valueRange.getEndInclusive().floatValue());
    }
