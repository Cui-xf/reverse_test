...
public final class MotionEventAdapter {
    ...
    private final long m5334getComposePointerId_I2yYro(int motionEventPointerId) {
        long newId;
        ...
        return PointerId.m5384constructorimpl(newId);
    }
    ...
    private final PointerInputEventData createPointerInputEventData(PositionCalculator positionCalculator, MotionEvent motionEvent, int index, boolean pressed) {
        ...
        long pointerId = m5334getComposePointerId_I2yYro(motionEventPointerId);
        float pressure = motionEvent.getPressure(index);
        long position2 = OffsetKt.Offset(motionEvent.getX(index), motionEvent.getY(index));
        long originalPositionEventPosition = Offset.m3939copydBAh8RU$default(position2, 0.0f, 0.0f, 3, null);
        if (index == 0) {
            long rawPosition2 = OffsetKt.Offset(motionEvent.getRawX(), motionEvent.getRawY());
            position = positionCalculator.mo5488screenToLocalMKHz9U(rawPosition2);
            rawPosition = rawPosition2;
        } else if (Build.VERSION.SDK_INT >= 29) {
            long rawPosition3 = MotionEventHelper.INSTANCE.m5335toRawOffsetdBAh8RU(motionEvent, index);
            position = positionCalculator.mo5488screenToLocalMKHz9U(rawPosition3);
            rawPosition = rawPosition3;
        } else {
            long rawPosition4 = positionCalculator.mo5487localToScreenMKHz9U(position2);
            position = position2;
            rawPosition = rawPosition4;
        }
        switch (motionEvent.getToolType(index)) {
            case 0:
                toolType = PointerType.INSTANCE.m5485getUnknownT8wyACA();
                break;
            case 1:
                toolType = PointerType.INSTANCE.m5484getTouchT8wyACA();
                break;
            case 2:
                toolType = PointerType.INSTANCE.m5483getStylusT8wyACA();
                break;
            case 3:
                toolType = PointerType.INSTANCE.m5482getMouseT8wyACA();
                break;
            case 4:
                toolType = PointerType.INSTANCE.m5481getEraserT8wyACA();
                break;
            default:
                toolType = PointerType.INSTANCE.m5485getUnknownT8wyACA();
                break;
        ...
        if (motionEvent.getActionMasked() == 8) {
            ...
            scrollDelta = Offset.INSTANCE.m3961getZeroF1C5BW0();
        }
