...
public final class AndroidMultiParagraphDraw_androidKt {
    ...
    public static final void m6400drawMultiParagraph7AXcY_I(MultiParagraph $this$drawMultiParagraph_u2d7AXcY_I, Canvas canvas, Brush brush, float alpha, Shadow shadow, TextDecoration decoration, DrawStyle drawStyle, int blendMode) {
        Canvas canvas2;
        canvas.save();
        if ($this$drawMultiParagraph_u2d7AXcY_I.getParagraphInfoList$ui_text_release().size() <= 1 || (brush instanceof SolidColor)) {
            m6402drawParagraphs7AXcY_I($this$drawMultiParagraph_u2d7AXcY_I, canvas, brush, alpha, shadow, decoration, drawStyle, blendMode);
            canvas2 = canvas;
            ...
            Shader shader = ((ShaderBrush) brush).mo4156createShaderuvyYCjk(SizeKt.Size(width, height));
            ...
            for (int index$iv2 = 0; index$iv2 < size2; index$iv2++) {
                ...
                it2.getParagraph().mo5984painthn5TExg(canvas, BrushKt.ShaderBrush(shader), alpha, shadow, decoration, drawStyle, blendMode);
    ...
    private static final void m6402drawParagraphs7AXcY_I(MultiParagraph $this$drawParagraphs_u2d7AXcY_I, Canvas canvas, Brush brush, float alpha, Shadow shadow, TextDecoration decoration, DrawStyle drawStyle, int blendMode) {
        List $this$fastForEach$iv = $this$drawParagraphs_u2d7AXcY_I.getParagraphInfoList$ui_text_release();
        int size = $this$fastForEach$iv.size();
        for (int index$iv = 0; index$iv < size; index$iv++) {
            Object item$iv = $this$fastForEach$iv.get(index$iv);
            ParagraphInfo it = (ParagraphInfo) item$iv;
            it.getParagraph().mo5984painthn5TExg(canvas, brush, alpha, shadow, decoration, drawStyle, blendMode);
