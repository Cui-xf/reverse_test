...
public final class MyersDiffKt {
    private static final IntStack calculateDiff(int oldSize, int newSize, DiffCallback cb) {
        ...
        int[] forward = CenteredArray.m5651constructorimpl(new int[(max * 2) + 1]);
        int[] backward = CenteredArray.m5651constructorimpl(new int[(max * 2) + 1]);
        int[] snake = Snake.m5838constructorimpl(new int[5]);
        while (stack.isNotEmpty()) {
            int newEnd = stack.pop();
            int newStart = stack.pop();
            int oldEnd = stack.pop();
            int oldStart = stack.pop();
            boolean found = m5737midPointq5eDKzI(oldStart, oldEnd, newStart, newEnd, cb, forward, backward, snake);
            if (found) {
                if (Snake.m5841getDiagonalSizeimpl(snake) > 0) {
                    Snake.m5836addDiagonalToStackimpl(snake, diagonals);
                }
                stack.pushRange(oldStart, Snake.m5846getStartXimpl(snake), newStart, Snake.m5847getStartYimpl(snake));
                stack.pushRange(Snake.m5842getEndXimpl(snake), oldEnd, Snake.m5843getEndYimpl(snake), newEnd);
            }
        }
    ...
    private static final boolean m5737midPointq5eDKzI(int oldStart, int oldEnd, int newStart, int newEnd, DiffCallback cb, int[] forward, int[] backward, int[] snake) {
        int i = oldStart;
        int i2 = oldEnd;
        int oldSize = i2 - i;
        ...
        int max = ((oldSize + newSize) + 1) / 2;
        int[] iArr = forward;
        CenteredArray.m5657setimpl(iArr, 1, i);
        int[] iArr2 = backward;
        CenteredArray.m5657setimpl(iArr2, 1, i2);
        int d = 0;
        while (d < max) {
            boolean found = m5736forward4l5_RBY(i, i2, newStart, newEnd, cb, iArr, iArr2, d, snake);
            if (found) {
                return true;
            }
            boolean found2 = m5735backward4l5_RBY(oldStart, oldEnd, newStart, newEnd, cb, forward, backward, d, snake);
            if (found2) {
                return true;
            }
            d++;
    ...
    private static final boolean m5736forward4l5_RBY(int oldStart, int oldEnd, int newStart, int newEnd, DiffCallback cb, int[] forward, int[] backward, int d, int[] snake) {
        int x;
        int startX;
        int i = oldEnd;
        ...
        int k = -d;
        while (k <= d) {
            if (k == (-d) || (k != d && CenteredArray.m5654getimpl(forward, k + 1) > CenteredArray.m5654getimpl(forward, k - 1))) {
                int startX2 = CenteredArray.m5654getimpl(forward, k + 1);
                x = startX2;
                startX = startX2;
            } else {
                int startX3 = CenteredArray.m5654getimpl(forward, k - 1);
                x = startX3 + 1;
                startX = startX3;
            }
            ...
            CenteredArray.m5657setimpl(forward, k, x);
            if (checkForSnake) {
                int backwardsK = delta - k;
                if (backwardsK >= (-d) + 1 && backwardsK <= d - 1) {
                    if (CenteredArray.m5654getimpl(backward, backwardsK) <= x) {
    ...
    private static final boolean m5735backward4l5_RBY(int oldStart, int oldEnd, int newStart, int newEnd, DiffCallback cb, int[] forward, int[] backward, int d, int[] snake) {
        int x;
        int startX;
        ...
        int delta = oldSize - newSize;
        for (int k = -d; k <= d; k += 2) {
            if (k == (-d) || (k != d && CenteredArray.m5654getimpl(backward, k + 1) < CenteredArray.m5654getimpl(backward, k - 1))) {
                int startX2 = CenteredArray.m5654getimpl(backward, k + 1);
                x = startX2;
                startX = startX2;
            } else {
                int startX3 = CenteredArray.m5654getimpl(backward, k - 1);
                x = startX3 - 1;
                startX = startX3;
            }
            ...
            CenteredArray.m5657setimpl(backward, k, x2);
            if (checkForSnake) {
                int forwardsK = delta - k;
                if (forwardsK >= (-d) && forwardsK <= d) {
                    if (CenteredArray.m5654getimpl(forward, forwardsK) >= x2) {
