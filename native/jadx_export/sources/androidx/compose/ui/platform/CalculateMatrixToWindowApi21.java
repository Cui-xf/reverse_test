...
final class CalculateMatrixToWindowApi21 implements CalculateMatrixToWindow {
    ...
    public void mo5876calculateMatrixToWindowEL8BTi8(View view, float[] matrix) {
        Matrix.m4433resetimpl(matrix);
        m5879transformMatrixToWindowEL8BTi8(view, matrix);
    }

    /* renamed from: transformMatrixToWindow-EL8BTi8, reason: not valid java name */
    private final void m5879transformMatrixToWindowEL8BTi8(View view, float[] matrix) {
        Object parent = view.getParent();
        if (parent instanceof View) {
            m5879transformMatrixToWindowEL8BTi8((View) parent, matrix);
            m5878preTranslate3XD1CNM(matrix, -view.getScrollX(), -view.getScrollY());
            m5878preTranslate3XD1CNM(matrix, view.getLeft(), view.getTop());
        } else {
            int[] pos = this.tmpLocation;
            view.getLocationInWindow(pos);
            m5878preTranslate3XD1CNM(matrix, -view.getScrollX(), -view.getScrollY());
            m5878preTranslate3XD1CNM(matrix, pos[0], pos[1]);
        }
        android.graphics.Matrix viewMatrix = view.getMatrix();
        if (!viewMatrix.isIdentity()) {
            m5877preConcattUYjHk(matrix, viewMatrix);
        }
    }

    /* renamed from: preConcat-tU-YjHk, reason: not valid java name */
    private final void m5877preConcattUYjHk(float[] $this$preConcat_u2dtU_u2dYjHk, android.graphics.Matrix other) {
        AndroidMatrixConversions_androidKt.m4058setFromtUYjHk(this.tmpMatrix, other);
        AndroidComposeView_androidKt.m5874preTransformJiSxe2E($this$preConcat_u2dtU_u2dYjHk, this.tmpMatrix);
    }

    /* renamed from: preTranslate-3XD1CNM, reason: not valid java name */
    private final void m5878preTranslate3XD1CNM(float[] $this$preTranslate_u2d3XD1CNM, float x, float y) {
        AndroidComposeView_androidKt.m5875preTranslatecG2Xzmc($this$preTranslate_u2d3XD1CNM, x, y, this.tmpMatrix);
    }
}
