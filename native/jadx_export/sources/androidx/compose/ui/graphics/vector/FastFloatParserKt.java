...
public final class FastFloatParserKt {
    ...
    public static final long nextFloat(String s, int start, int end) throws NumberFormatException {
        long j;
        char c;
        char c2;
        ...
        long significand3;
        if (start == end) {
            return (4294967295L & Float.floatToRawIntBits(Float.NaN)) | (start << 32);
        }
        int index = start;
        char c4 = s.charAt(index);
        boolean isNegative = c4 == '-';
        if (isNegative) {
            index++;
            if (index == end) {
                return (4294967295L & Float.floatToRawIntBits(Float.NaN)) | (index << 32);
            }
            c4 = s.charAt(index);
            j = 4294967295L;
            if (!(((char) (c4 + 65488)) < '\n') && c4 != '.') {
                return (index << 32) | (Float.floatToRawIntBits(Float.NaN) & 4294967295L);
            }
        } else {
            j = 4294967295L;
        }
        ...
        while (true) {
            ...
            char $this$isDigit$iv = c4;
            c2 = ' ';
            if (!(((char) ($this$isDigit$iv + 65488)) < '\n')) {
                c = 1;
                break;
            }
            significand4 = (10 * significand4) + (c4 - '0');
            index++;
            c4 = index < dataLength ? s.charAt(index) : (char) 0;
        }
        int significandEndIndex = index;
        int digitCount = index - digits;
        int exponent = 0;
        ...
        if (index == end || c4 != '.') {
            ...
            while (true) {
                ...
                significandStartIndex = digits;
                long jCharAt = s.charAt(index);
                significand2 = significand4;
                long significand5 = s.charAt(index + 1);
                long v$iv = (significand5 << 16) | jCharAt | (s.charAt(index + 2) << c2) | (s.charAt(index + 3) << 48);
                long base$iv = v$iv - 13511005043687472L;
                long predicate$iv = (v$iv + 19703549022044230L) | base$iv;
                int digits2 = (predicate$iv & (-35747867511423104L)) != 0 ? -1 : (int) ((281475406208040961L * base$iv) >>> 48);
                if (digits2 < 0) {
                    break;
                }
                significand4 = (10000 * significand2) + digits2;
                index += 4;
                digits = significandStartIndex;
            }
            c4 = index < dataLength ? s.charAt(index) : (char) 0;
            long significand6 = significand2;
            while (true) {
                if (index == end) {
                    significand3 = significand6;
                    break;
                }
                char $this$isDigit$iv2 = c4;
                char $this$isDigit$iv3 = ((char) ($this$isDigit$iv2 + 65488)) < '\n' ? (char) 1 : (char) 0;
                if ($this$isDigit$iv3 == 0) {
                    significand3 = significand6;
                    break;
                }
                long j3 = significand6 * 10;
                long significand7 = c4 - '0';
                significand6 = significand7 + j3;
                index++;
                c4 = index < dataLength ? s.charAt(index) : (char) 0;
            }
        ...
        if (digitCount == 0) {
            long significand8 = index;
            return (significand8 << c2) | (Float.floatToRawIntBits(Float.NaN) & j);
        }
        long significand9 = significand4;
        int exponentNumber2 = 0;
        if ((c4 | ' ') == 101) {
            index++;
            char c5 = index < dataLength ? s.charAt(index) : (char) 0;
            ...
            while (true) {
                ...
                char $this$isDigit$iv4 = c6;
                c3 = c6;
                char $this$isDigit$iv5 = ((char) ($this$isDigit$iv4 + 65488)) < '\n' ? (char) 1 : (char) 0;
                if ($this$isDigit$iv5 == 0) {
                    break;
                }
                if (exponentNumber2 < 1024) {
                    exponentNumber2 = (exponentNumber2 * 10) + (c3 - '0');
                }
                index++;
        ...
        if (digitCount > 19) {
            ...
            c4 = s.charAt(retryIndex2);
            while (index != end && (c4 == '0' || c4 == '.')) {
                if (c4 == '0') {
                    digitCount--;
                }
                retryIndex2++;
                c4 = retryIndex2 < dataLength ? s.charAt(retryIndex2) : (char) 0;
            }
            if (digitCount > 19) {
                long significand10 = 0;
                int retryIndex3 = significandStartIndex;
                c4 = s.charAt(retryIndex3);
                while (true) {
                    j2 = 1000000000000000000L;
                    if (retryIndex3 == significandEndIndex) {
                        exponentNumber = exponentNumber2;
                        break;
                    }
                    exponentNumber = exponentNumber2;
                    if (Long.compare(ULong.m7173constructorimpl(significand10) ^ Long.MIN_VALUE, 1000000000000000000L ^ Long.MIN_VALUE) >= 0) {
                        break;
                    }
                    significand10 = (significand10 * 10) + (c4 - '0');
                    retryIndex3++;
                    c4 = retryIndex3 < dataLength ? s.charAt(retryIndex3) : (char) 0;
                    exponentNumber2 = exponentNumber;
                }
                if (Long.compare(ULong.m7173constructorimpl(significand10) ^ Long.MIN_VALUE, 1000000000000000000L ^ Long.MIN_VALUE) >= 0) {
                    exponent = (significandEndIndex - retryIndex3) + exponentNumber;
                    tooManyDigits = true;
                    significand = significand10;
                } else {
                    int retryIndex4 = exponentStartIndex;
                    c4 = s.charAt(retryIndex4);
                    while (true) {
                        if (retryIndex4 == exponentEndIndex) {
                            retryIndex = retryIndex4;
                            break;
                        }
                        retryIndex = retryIndex4;
                        if (Long.compare(ULong.m7173constructorimpl(significand10) ^ Long.MIN_VALUE, j2 ^ Long.MIN_VALUE) >= 0) {
                            break;
                        }
                        significand10 = (significand10 * 10) + (c4 - '0');
                        retryIndex4 = retryIndex + 1;
                        c4 = retryIndex4 < dataLength ? s.charAt(retryIndex4) : (char) 0;
                        j2 = 1000000000000000000L;
                    }
        ...
        if ((FloatMinExponent <= exponent && exponent < 11) && !tooManyDigits && Long.compare(ULong.m7173constructorimpl(significand) ^ Long.MIN_VALUE, ULong.m7173constructorimpl(16777216L) ^ Long.MIN_VALUE) <= 0) {
            float f = significand;
            float f2 = exponent < 0 ? f / PowersOfTen[-exponent] : f * PowersOfTen[exponent];
            float value$iv = isNegative ? -f2 : f2;
            return (index << c2) | (Float.floatToRawIntBits(value$iv) & j);
        }
        long significand11 = significand;
        boolean tooManyDigits2 = tooManyDigits;
        if (significand11 == 0) {
            float value$iv2 = isNegative ? -0.0f : 0.0f;
            return (index << c2) | (Float.floatToRawIntBits(value$iv2) & j);
        }
        if (!(-126 <= exponent && exponent < 128)) {
            ...
            return (index << c2) | (Float.floatToRawIntBits(value$iv3) & j);
        }
        long significandFactor = ULongArray.m7233getsVKNKU(Mantissa64, exponent + 325);
        int lz = Long.numberOfLeadingZeros(significand11);
        long significand12 = significand11 << lz;
        long xLo$iv = significand12 & j;
        long xHi$iv = significand12 >>> c2;
        long yLo$iv = significandFactor & j;
        long yHi$iv = significandFactor >>> c2;
        ...
        long carry$iv = yTimesXMid$iv + (xTimesYLo$iv >>> c2) + (xTimesYMid$iv & j);
        long upper = xTimesYHi$iv + (carry$iv >>> c2) + (xTimesYMid$iv >>> c2);
        ...
        if ((upper & 511) == 511 || ((upper & 511) == 0 && (mantissa & 3) == 1)) {
            ...
            return (index << c2) | (Float.floatToRawIntBits(value$iv4) & j);
        }
        long mantissa2 = (mantissa + 1) >>> c;
        if (mantissa2 >= 9007199254740992L) {
            mantissa2 = 4503599627370496L;
            lz2--;
        }
        long mantissa3 = mantissa2 & (-4503599627370497L);
        long adjustedExponent = ((exponent * 217706) >> 16) + 1024 + 63;
    ...
    private static final long pack(int index, float value) {
        return (index << 32) | (Float.floatToRawIntBits(value) & 4294967295L);
    }
