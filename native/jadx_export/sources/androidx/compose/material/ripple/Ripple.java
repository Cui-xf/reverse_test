...
public abstract class Ripple implements Indication {
    ...
    public abstract RippleIndicationInstance mo1756rememberUpdatedRippleInstance942rkJo(InteractionSource interactionSource, boolean z, float f, State<Color> state, State<RippleAlpha> state2, Composer composer, int i);
    ...
    public final IndicationInstance rememberUpdatedInstance(InteractionSource interactionSource, Composer $composer, int $changed) {
        long jMo1517defaultColorWaAFU9c;
        ...
        long $this$isSpecified$iv = this.color.getValue().m4197unboximpl();
        boolean z = true;
        if ($this$isSpecified$iv != Color.INSTANCE.m4223getUnspecified0d7_KjU()) {
            $composer.startReplaceGroup(-303571590);
            $composer.endReplaceGroup();
            jMo1517defaultColorWaAFU9c = this.color.getValue().m4197unboximpl();
        } else {
            $composer.startReplaceGroup(-303521246);
            ComposerKt.sourceInformation($composer, "203@9725L14");
            jMo1517defaultColorWaAFU9c = theme.mo1517defaultColorWaAFU9c($composer, 0);
            $composer.endReplaceGroup();
        }
        State color = SnapshotStateKt.rememberUpdatedState(Color.m4177boximpl(jMo1517defaultColorWaAFU9c), $composer, 0);
        State rippleAlpha = SnapshotStateKt.rememberUpdatedState(theme.rippleAlpha($composer, 0), $composer, 0);
        RippleIndicationInstance instance = mo1756rememberUpdatedRippleInstance942rkJo(interactionSource, this.bounded, this.radius, color, rippleAlpha, $composer, ($changed & 14) | (($changed << 12) & 458752));
    ...
    public boolean equals(Object other) {
        ...
        return (other instanceof Ripple) && this.bounded == ((Ripple) other).bounded && Dp.m6698equalsimpl0(this.radius, ((Ripple) other).radius) && Intrinsics.areEqual(this.color, ((Ripple) other).color);
    }

    public int hashCode() {
        int result = ChangeSize$$ExternalSyntheticBackport0.m(this.bounded);
        return (((result * 31) + Dp.m6699hashCodeimpl(this.radius)) * 31) + this.color.hashCode();
    }
}
