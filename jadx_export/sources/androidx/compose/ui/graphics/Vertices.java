...
public final class Vertices {
    ...
    private final int[] encodeColorList(List<Color> colors) {
        ...
        for (int i = 0; i < size; i++) {
            iArr[i] = ColorKt.m4241toArgb8_81llA(colors.get(i).m4197unboximpl());
        }
        return iArr;
    }

    private final float[] encodePointList(List<Offset> points) {
        float fM3946getYimpl;
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (i % 2 == 0) {
                fM3946getYimpl = Offset.m3945getXimpl(point);
            } else {
                fM3946getYimpl = Offset.m3946getYimpl(point);
            }
            fArr[i] = fM3946getYimpl;
        }
        return fArr;
    }
}
