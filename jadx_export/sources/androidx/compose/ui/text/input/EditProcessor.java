...
public final class EditProcessor {
    ...
    private TextFieldValue mBufferState = new TextFieldValue(AnnotatedStringKt.emptyAnnotatedString(), TextRange.INSTANCE.m6151getZerod9O1mEE(), (TextRange) null, (DefaultConstructorMarker) null);
    ...
    public final void reset(TextFieldValue value, TextInputSession textInputSession) {
        ...
        boolean compositionChanged = !Intrinsics.areEqual(value.getComposition(), this.mBuffer.m6307getCompositionMzsxiRA$ui_text_release());
        if (!Intrinsics.areEqual(this.mBufferState.getText(), value.getText())) {
            ...
            textChanged = true;
        } else if (!TextRange.m6139equalsimpl0(this.mBufferState.getSelection(), value.getSelection())) {
            this.mBuffer.setSelection$ui_text_release(TextRange.m6144getMinimpl(value.getSelection()), TextRange.m6143getMaximpl(value.getSelection()));
            selectionChanged = true;
        }
        if (value.getComposition() == null) {
            this.mBuffer.commitComposition$ui_text_release();
        } else if (!TextRange.m6140getCollapsedimpl(value.getComposition().getPackedValue())) {
            this.mBuffer.setComposition$ui_text_release(TextRange.m6144getMinimpl(value.getComposition().getPackedValue()), TextRange.m6143getMaximpl(value.getComposition().getPackedValue()));
        }
        if (textChanged || (!selectionChanged && compositionChanged)) {
            this.mBuffer.commitComposition$ui_text_release();
            value = TextFieldValue.m6390copy3r_uNRQ$default(value, (AnnotatedString) null, 0L, (TextRange) null, 3, (Object) null);
        }
    ...
    public final TextFieldValue apply(List<? extends EditCommand> editCommands) {
        ...
        try {
            ...
            long $this$apply_u24lambda_u242 = this.mBuffer.m6308getSelectiond9O1mEE$ui_text_release();
            TextRange textRangeM6134boximpl = TextRange.m6134boximpl($this$apply_u24lambda_u242);
            textRangeM6134boximpl.getPackedValue();
            if (TextRange.m6145getReversedimpl(this.mBufferState.getSelection())) {
                textRangeM6134boximpl = null;
            }
            TextFieldValue newState = new TextFieldValue(annotatedString$ui_text_release, textRangeM6134boximpl != null ? textRangeM6134boximpl.getPackedValue() : TextRangeKt.TextRange(TextRange.m6143getMaximpl($this$apply_u24lambda_u242), TextRange.m6144getMinimpl($this$apply_u24lambda_u242)), this.mBuffer.m6307getCompositionMzsxiRA$ui_text_release(), (DefaultConstructorMarker) null);
    ...
    private final String generateBatchErrorMessage(List<? extends EditCommand> editCommands, final EditCommand failedCommand) {
        ...
        StringBuilder sbAppend = $this$generateBatchErrorMessage_u24lambda_u243.append("Error while applying EditCommand batch to buffer (length=" + this.mBuffer.getLength$ui_text_release() + ", composition=" + this.mBuffer.m6307getCompositionMzsxiRA$ui_text_release() + ", selection=" + ((Object) TextRange.m6149toStringimpl(this.mBuffer.m6308getSelectiond9O1mEE$ui_text_release())) + "):");
