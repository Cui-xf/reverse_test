    ...
    private static final boolean calculateIfHorizontallyStacked(List<SemanticsNode> list) {
        ...
        if (list.size() == 0 || list.size() == 1) {
            ...
            for (int i$iv = 0; i$iv < lastIndex; i$iv++) {
                ...
                result$iv.add(Offset.m3934boximpl(OffsetKt.Offset(Math.abs(Offset.m3945getXimpl(el1.getBoundsInRoot().m3975getCenterF1C5BW0()) - Offset.m3945getXimpl(el2.getBoundsInRoot().m3975getCenterF1C5BW0())), Math.abs(Offset.m3946getYimpl(el1.getBoundsInRoot().m3975getCenterF1C5BW0()) - Offset.m3946getYimpl(el2.getBoundsInRoot().m3975getCenterF1C5BW0())))));
                current$iv = next$iv;
            }
        }
        if (result$iv.size() == 1) {
            ...
            if (1 <= lastIndex2) {
                while (true) {
                    ...
                    accumulator$iv = Offset.m3934boximpl(Offset.m3950plusMKHz9U(result, element));
                    if (i$iv2 == lastIndex2) {
                        break;
                    }
                    i$iv2++;
        ...
        float deltaX = Offset.m3945getXimpl(packedValue);
        float deltaY = Offset.m3946getYimpl(packedValue);
