...
public abstract class LazyStaggeredGridMeasureProvider implements LazyLayoutMeasuredItemProvider<LazyStaggeredGridMeasuredItem> {
    ...
    public abstract LazyStaggeredGridMeasuredItem mo896createItempitSLOA(int index, int lane, int span, Object key, Object contentType, List<? extends Placeable> placeables, long constraints);
    ...
    private final long m902childConstraintsJhjzzOo(int slot, int span) {
        int start;
        ...
        if (this.isVertical) {
            return Constraints.INSTANCE.m6648fixedWidthOenEA2s(start);
        }
        return Constraints.INSTANCE.m6647fixedHeightOenEA2s(start);
    }

    /* renamed from: getAndMeasure-jy6DScQ, reason: not valid java name */
    public final LazyStaggeredGridMeasuredItem m903getAndMeasurejy6DScQ(int index, long span) {
        ...
        long constraints = m902childConstraintsJhjzzOo(spanStart, spanSize);
        List placeables = this.measureScope.mo851measure0kLqBqw(index, constraints);
        return mo896createItempitSLOA(index, spanStart, spanSize, key, contentType, placeables, constraints);
    }

    @Override // androidx.compose.foundation.lazy.layout.LazyLayoutMeasuredItemProvider
    /* renamed from: getAndMeasure--hBUhpc */
    public LazyStaggeredGridMeasuredItem mo794getAndMeasurehBUhpc(int index, int lane, int span, long constraints) {
        Object key = this.itemProvider.getKey(index);
        Object contentType = this.itemProvider.getContentType(index);
        List placeables = this.measureScope.mo851measure0kLqBqw(index, constraints);
        return mo896createItempitSLOA(index, lane, span, key, contentType, placeables, constraints);
    }
