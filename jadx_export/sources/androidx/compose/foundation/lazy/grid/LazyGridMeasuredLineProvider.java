...
public abstract class LazyGridMeasuredLineProvider {
    ...
    public final long m825childConstraintsJhjzzOo$foundation_release(int startSlot, int span) {
        int i;
        ...
        if (this.isVertical) {
            return Constraints.INSTANCE.m6648fixedWidthOenEA2s(crossAxisSize);
        }
        return Constraints.INSTANCE.m6647fixedHeightOenEA2s(crossAxisSize);
    }
    ...
    public final LazyGridMeasuredLine getAndMeasure(int lineIndex) {
        ...
        for (int i2 = 0; i2 < lineItemsCount; i2++) {
            int span = GridItemSpan.m803getCurrentLineSpanimpl(lineConfiguration.getSpans().get(i2).getPackedValue());
            long constraints = m825childConstraintsJhjzzOo$foundation_release(startSlot, span);
            LazyGridMeasuredItem lazyGridMeasuredItemM824getAndMeasurem8Kt_7k = this.measuredItemProvider.m824getAndMeasurem8Kt_7k(lineConfiguration.getFirstItemIndex() + i2, constraints, startSlot, span, mainAxisSpacing);
            startSlot += span;
            Unit unit = Unit.INSTANCE;
            items[i2] = lazyGridMeasuredItemM824getAndMeasurem8Kt_7k;
        }
