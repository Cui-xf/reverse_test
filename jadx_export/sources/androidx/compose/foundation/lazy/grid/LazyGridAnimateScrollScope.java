...
public final class LazyGridAnimateScrollScope implements LazyLayoutAnimateScrollScope {
    ...
    public float calculateDistanceTo(int targetIndex) {
        ...
        if (layoutInfo.getOrientation() == Orientation.Vertical) {
            iM6825getXimpl = IntOffset.m6826getYimpl(visibleItem.getOffset());
        } else {
            iM6825getXimpl = IntOffset.m6825getXimpl(visibleItem.getOffset());
        }
        return iM6825getXimpl;
    }

    private final int calculateLineAverageMainAxisSize(LazyGridLayoutInfo layoutInfo) {
        int iM6867getWidthimpl;
        ...
        while (lineStartIndex < visibleItems.size()) {
            ...
            if (currentLine == -1) {
                ...
                while (lineEndIndex < visibleItems.size() && lineOf.invoke(Integer.valueOf(lineEndIndex)).intValue() == currentLine) {
                    if (isVertical) {
                        iM6867getWidthimpl = IntSize.m6866getHeightimpl(visibleItems.get(lineEndIndex).getSize());
                    } else {
                        iM6867getWidthimpl = IntSize.m6867getWidthimpl(visibleItems.get(lineEndIndex).getSize());
                    }
                    lineMainAxisSize = Math.max(lineMainAxisSize, iM6867getWidthimpl);
                    lineEndIndex++;
                }
