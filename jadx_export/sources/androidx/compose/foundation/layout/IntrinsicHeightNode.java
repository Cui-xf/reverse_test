...
final class IntrinsicHeightNode extends IntrinsicSizeModifier {
    ...
    public long mo635calculateContentConstraintsl58MMJ0(MeasureScope $this$calculateContentConstraints_u2dl58MMJ0, Measurable measurable, long constraints) {
        ...
        if (this.height == IntrinsicSize.Min) {
            measuredHeight = measurable.minIntrinsicHeight(Constraints.m6636getMaxWidthimpl(constraints));
        } else {
            measuredHeight = measurable.maxIntrinsicHeight(Constraints.m6636getMaxWidthimpl(constraints));
        }
        if (measuredHeight < 0) {
            measuredHeight = 0;
        }
        return Constraints.INSTANCE.m6647fixedHeightOenEA2s(measuredHeight);
    }
