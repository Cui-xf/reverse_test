...
public final class AndroidEdgeEffectOverscrollEffect implements OverscrollEffect {
    ...
    private long containerSize = Size.INSTANCE.m4023getZeroNHjbRc();
    ...
    public AndroidEdgeEffectOverscrollEffect(Context context, OverscrollConfiguration overscrollConfig) {
        ...
        this.edgeEffectWrapper = new EdgeEffectWrapper(context, ColorKt.m4241toArgb8_81llA(overscrollConfig.getGlowColor()));
    ...
    public long mo212applyToScrollRhakbz0(long delta, int source, Function1<? super Offset, Offset> performScroll) {
        float consumedPixelsY;
        boolean appliedHorizontalOverscroll;
        boolean appliedVerticalOverscroll;
        if (Size.m4016isEmptyimpl(this.containerSize)) {
            return performScroll.invoke(Offset.m3934boximpl(delta)).getPackedValue();
        }
        ...
        if (Offset.m3946getYimpl(delta) == 0.0f) {
            consumedPixelsY = 0.0f;
        } else if (this.edgeEffectWrapper.isTopStretched()) {
            consumedPixelsY = m209pullTopk4lQ0M(delta);
            ...
            consumedPixelsY = m206pullBottomk4lQ0M(delta);
        ...
        if (!Offset.m3942equalsimpl0(consumedOffset, Offset.INSTANCE.m3961getZeroF1C5BW0())) {
            invalidateOverscroll$foundation_release();
        }
        long leftForDelta = Offset.m3949minusMKHz9U(delta, consumedOffset);
        long consumedByDelta = performScroll.invoke(Offset.m3934boximpl(leftForDelta)).getPackedValue();
        long leftForOverscroll = Offset.m3949minusMKHz9U(leftForDelta, consumedByDelta);
        ...
        if (NestedScrollSource.m5286equalsimpl0(source, NestedScrollSource.INSTANCE.m5298getUserInputWNlRxjI())) {
            if (Offset.m3945getXimpl(leftForOverscroll) > 0.5f) {
                m207pullLeftk4lQ0M(leftForOverscroll);
                appliedHorizontalOverscroll = true;
            } else if (Offset.m3945getXimpl(leftForOverscroll) < -0.5f) {
                m208pullRightk4lQ0M(leftForOverscroll);
                appliedHorizontalOverscroll = true;
            } else {
                appliedHorizontalOverscroll = false;
            }
            if (Offset.m3946getYimpl(leftForOverscroll) > 0.5f) {
                m209pullTopk4lQ0M(leftForOverscroll);
                appliedVerticalOverscroll = true;
            } else if (Offset.m3946getYimpl(leftForOverscroll) < -0.5f) {
                m206pullBottomk4lQ0M(leftForOverscroll);
                appliedVerticalOverscroll = true;
            } else {
        ...
        boolean appliedHorizontalOverscroll2 = m210releaseOppositeOverscrollk4lQ0M(delta);
        boolean needsInvalidation2 = appliedHorizontalOverscroll2 || needsInvalidation;
        if (needsInvalidation2) {
            invalidateOverscroll$foundation_release();
        }
        return Offset.m3950plusMKHz9U(consumedOffset, consumedByDelta);
    }
    ...
    */
    public java.lang.Object mo211applyToFlingBMRW4eQ(long r9, kotlin.jvm.functions.Function2<? super androidx.compose.ui.unit.Velocity, ? super kotlin.coroutines.Continuation<? super androidx.compose.ui.unit.Velocity>, ? extends java.lang.Object> r11, kotlin.coroutines.Continuation<? super kotlin.Unit> r12) {
        /*
            Method dump skipped, instructions count: 454
            To view this dump change 'Code comments level' option to 'DEBUG'
        */
        throw new UnsupportedOperationException("Method not decompiled: androidx.compose.foundation.AndroidEdgeEffectOverscrollEffect.mo211applyToFlingBMRW4eQ(long, kotlin.jvm.functions.Function2, kotlin.coroutines.Continuation):java.lang.Object");
    }
    ...
    private final boolean stopOverscrollAnimation() {
        ...
        if (this.edgeEffectWrapper.isLeftStretched()) {
            m207pullLeftk4lQ0M(Offset.INSTANCE.m3961getZeroF1C5BW0());
            stopped = true;
        }
        if (this.edgeEffectWrapper.isRightStretched()) {
            m208pullRightk4lQ0M(Offset.INSTANCE.m3961getZeroF1C5BW0());
            stopped = true;
        }
        if (this.edgeEffectWrapper.isTopStretched()) {
            m209pullTopk4lQ0M(Offset.INSTANCE.m3961getZeroF1C5BW0());
            stopped = true;
        }
        if (this.edgeEffectWrapper.isBottomStretched()) {
            m206pullBottomk4lQ0M(Offset.INSTANCE.m3961getZeroF1C5BW0());
            return true;
        }
        return stopped;
    }
    ...
    public final void m214updateSizeuvyYCjk$foundation_release(long size) {
        boolean initialSetSize = Size.m4010equalsimpl0(this.containerSize, Size.INSTANCE.m4023getZeroNHjbRc());
        boolean differentSize = !Size.m4010equalsimpl0(size, this.containerSize);
        this.containerSize = size;
        if (differentSize) {
            this.edgeEffectWrapper.m285setSizeozmzZPI(IntSizeKt.IntSize(MathKt.roundToInt(Size.m4014getWidthimpl(size)), MathKt.roundToInt(Size.m4011getHeightimpl(size))));
        }
    ...
    public final long m213displacementF1C5BW0$foundation_release() {
        Offset offset = this.pointerPosition;
        long pointer = offset != null ? offset.getPackedValue() : SizeKt.m4024getCenteruvyYCjk(this.containerSize);
        float x = Offset.m3945getXimpl(pointer) / Size.m4014getWidthimpl(this.containerSize);
        float y = Offset.m3946getYimpl(pointer) / Size.m4011getHeightimpl(this.containerSize);
    ...
    private final boolean m210releaseOppositeOverscrollk4lQ0M(long delta) {
        boolean needsInvalidation = false;
        boolean needsInvalidation2 = true;
        if (this.edgeEffectWrapper.isLeftAnimating() && Offset.m3945getXimpl(delta) < 0.0f) {
            EdgeEffectCompat.INSTANCE.onReleaseWithOppositeDelta(this.edgeEffectWrapper.getOrCreateLeftEffect(), Offset.m3945getXimpl(delta));
            needsInvalidation = !this.edgeEffectWrapper.isLeftAnimating();
        }
        if (this.edgeEffectWrapper.isRightAnimating() && Offset.m3945getXimpl(delta) > 0.0f) {
            EdgeEffectCompat.INSTANCE.onReleaseWithOppositeDelta(this.edgeEffectWrapper.getOrCreateRightEffect(), Offset.m3945getXimpl(delta));
            needsInvalidation = needsInvalidation || !this.edgeEffectWrapper.isRightAnimating();
        }
        if (this.edgeEffectWrapper.isTopAnimating() && Offset.m3946getYimpl(delta) < 0.0f) {
            EdgeEffectCompat.INSTANCE.onReleaseWithOppositeDelta(this.edgeEffectWrapper.getOrCreateTopEffect(), Offset.m3946getYimpl(delta));
            needsInvalidation = needsInvalidation || !this.edgeEffectWrapper.isTopAnimating();
        }
        if (this.edgeEffectWrapper.isBottomAnimating() && Offset.m3946getYimpl(delta) > 0.0f) {
            EdgeEffectCompat.INSTANCE.onReleaseWithOppositeDelta(this.edgeEffectWrapper.getOrCreateBottomEffect(), Offset.m3946getYimpl(delta));
    ...
    private final float m209pullTopk4lQ0M(long scroll) {
        float displacementX = Offset.m3945getXimpl(m213displacementF1C5BW0$foundation_release());
        float pullY = Offset.m3946getYimpl(scroll) / Size.m4011getHeightimpl(this.containerSize);
        EdgeEffect topEffect = this.edgeEffectWrapper.getOrCreateTopEffect();
        float consumed = EdgeEffectCompat.INSTANCE.onPullDistanceCompat(topEffect, pullY, displacementX) * Size.m4011getHeightimpl(this.containerSize);
        if (!(EdgeEffectCompat.INSTANCE.getDistanceCompat(topEffect) == 0.0f)) {
            return Offset.m3946getYimpl(scroll);
        }
        return consumed;
    }

    /* renamed from: pullBottom-k-4lQ0M, reason: not valid java name */
    private final float m206pullBottomk4lQ0M(long scroll) {
        float displacementX = Offset.m3945getXimpl(m213displacementF1C5BW0$foundation_release());
        float pullY = Offset.m3946getYimpl(scroll) / Size.m4011getHeightimpl(this.containerSize);
        EdgeEffect bottomEffect = this.edgeEffectWrapper.getOrCreateBottomEffect();
        float consumed = (-EdgeEffectCompat.INSTANCE.onPullDistanceCompat(bottomEffect, -pullY, 1.0f - displacementX)) * Size.m4011getHeightimpl(this.containerSize);
    ...
    private final float m207pullLeftk4lQ0M(long scroll) {
        float displacementY = Offset.m3946getYimpl(m213displacementF1C5BW0$foundation_release());
        float pullX = Offset.m3945getXimpl(scroll) / Size.m4014getWidthimpl(this.containerSize);
        EdgeEffect leftEffect = this.edgeEffectWrapper.getOrCreateLeftEffect();
        float consumed = EdgeEffectCompat.INSTANCE.onPullDistanceCompat(leftEffect, pullX, 1.0f - displacementY) * Size.m4014getWidthimpl(this.containerSize);
        if (!(EdgeEffectCompat.INSTANCE.getDistanceCompat(leftEffect) == 0.0f)) {
            return Offset.m3945getXimpl(scroll);
        }
        return consumed;
    }

    /* renamed from: pullRight-k-4lQ0M, reason: not valid java name */
    private final float m208pullRightk4lQ0M(long scroll) {
        float displacementY = Offset.m3946getYimpl(m213displacementF1C5BW0$foundation_release());
        float pullX = Offset.m3945getXimpl(scroll) / Size.m4014getWidthimpl(this.containerSize);
        EdgeEffect rightEffect = this.edgeEffectWrapper.getOrCreateRightEffect();
        float consumed = (-EdgeEffectCompat.INSTANCE.onPullDistanceCompat(rightEffect, -pullX, displacementY)) * Size.m4014getWidthimpl(this.containerSize);
