...
public final class EditingBuffer {
    ...
    private EditingBuffer(AnnotatedString annotatedString, long j) {
        ...
        this.selectionStart = TextRange.m6146getStartimpl(j);
        this.selectionEnd = TextRange.m6141getEndimpl(j);
        this.compositionStart = -1;
        this.compositionEnd = -1;
        checkRange(TextRange.m6146getStartimpl(j), TextRange.m6141getEndimpl(j));
    }
    ...
    public final TextRange m1144getCompositionMzsxiRA() {
        if (hasComposition()) {
            return TextRange.m6134boximpl(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd));
        }
        return null;
    }
    ...
    public final void delete(int start, int end) {
        ...
        PartialGapBuffer.replace$default(this.gapBuffer, TextRange.m6144getMinimpl(deleteRange), TextRange.m6143getMaximpl(deleteRange), "", 0, 0, 24, null);
        long newSelection = EditingBufferKt.m1147updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.selectionStart, this.selectionEnd), deleteRange);
        setSelectionStart(TextRange.m6146getStartimpl(newSelection));
        setSelectionEnd(TextRange.m6141getEndimpl(newSelection));
        if (hasComposition()) {
            long compositionRange = TextRangeKt.TextRange(this.compositionStart, this.compositionEnd);
            long newComposition = EditingBufferKt.m1147updateRangeAfterDeletepWDy79M(compositionRange, deleteRange);
            if (TextRange.m6140getCollapsedimpl(newComposition)) {
                commitComposition();
            } else {
                this.compositionStart = TextRange.m6144getMinimpl(newComposition);
                this.compositionEnd = TextRange.m6143getMaximpl(newComposition);
            }
        }
        this.highlight = null;
    }
    ...
    public final void m1146setHighlightK7f2yys(int type, int start, int end) {
        ...
        int clampedStart = RangesKt.coerceIn(start, 0, getLength());
        int clampedEnd = RangesKt.coerceIn(end, 0, getLength());
        this.highlight = new Pair<>(TextHighlightType.m1119boximpl(type), TextRange.m6134boximpl(TextRangeKt.TextRange(clampedStart, clampedEnd)));
    }
