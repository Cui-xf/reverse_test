...
public final class OffsetMappingCalculator {
    ...
    private int[] ops = OpArray.m1184constructorimpl(10);
    private int opsSize;
    ...
    public final void recordEditOperation(int sourceStart, int sourceEnd, int newLength) {
        ...
        if (sourceLength >= 2 || sourceLength != newLength) {
            ...
            if (newSize > OpArray.m1191getSizeimpl(this.ops)) {
                int newCapacity = Math.max(newSize * 2, OpArray.m1191getSizeimpl(this.ops) * 2);
                this.ops = OpArray.m1186copyOfpSmdads(this.ops, newCapacity);
            }
            OpArray.m1193setimpl(this.ops, this.opsSize, sourceMin, sourceLength, newLength);
            this.opsSize = newSize;
        }
    }

    /* renamed from: mapFromSource--jx7JFs, reason: not valid java name */
    public final long m1182mapFromSourcejx7JFs(int offset) {
        return m1179mapfzxv0v0(offset, true);
    }

    /* renamed from: mapFromDest--jx7JFs, reason: not valid java name */
    public final long m1181mapFromDestjx7JFs(int offset) {
        return m1179mapfzxv0v0(offset, false);
    }

    /* renamed from: map-fzxv0v0, reason: not valid java name */
    private final long m1179mapfzxv0v0(int offset, boolean fromSource) {
        ...
        if (max$iv >= 0) {
            if (reversed$iv) {
                ...
                while (-1 < i$iv) {
                    ...
                    long newStart = offsetMappingCalculator.m1180mapStepC6uMEY(start, offset$iv, srcLen$iv, destLen$iv, fromSource);
                    long newEnd = m1180mapStepC6uMEY(end2, offset$iv, srcLen$iv, destLen$iv, fromSource);
                    int start2 = Math.min(TextRange.m6146getStartimpl(newStart), TextRange.m6146getStartimpl(newEnd));
                    end2 = Math.max(TextRange.m6141getEndimpl(newStart), TextRange.m6141getEndimpl(newEnd));
                    i$iv--;
                    start = start2;
                ...
                while (i$iv2 < max$iv) {
                    ...
                    long newStart2 = m1180mapStepC6uMEY(start, offset$iv2, srcLen$iv2, destLen$iv2, fromSource);
                    long newEnd2 = m1180mapStepC6uMEY(end3, offset$iv2, srcLen$iv2, destLen$iv2, fromSource);
                    int start3 = Math.min(TextRange.m6146getStartimpl(newStart2), TextRange.m6146getStartimpl(newEnd2));
                    end3 = Math.max(TextRange.m6141getEndimpl(newStart2), TextRange.m6141getEndimpl(newEnd2));
                    i$iv2++;
                    start = start3;
                }
                end = end3;
    ...
    private final long m1180mapStepC6uMEY(int offset, int opOffset, int untransformedLen, int transformedLen, boolean fromSource) {
