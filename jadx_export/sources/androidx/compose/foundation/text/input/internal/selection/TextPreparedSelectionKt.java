...
public final class TextPreparedSelectionKt {
    ...
    public static final int calculateAdjacentCursorPosition(String transformedText, int cursor, boolean forward, TransformedTextFieldState state) {
        ...
        long untransformed$iv = state.m1230mapFromTransformedjx7JFs(proposedCursor);
        long retransformed$iv = state.m1233mapToTransformedGEjPoXI(untransformed$iv);
        ...
        switch (WhenMappings.$EnumSwitchMapping$0[type.ordinal()]) {
            ...
            case 3:
                return forward ? TextRange.m6141getEndimpl(retransformed$iv) : TextRange.m6146getStartimpl(retransformed$iv);
            case 4:
                if (forward) {
                    if (proposedCursor == TextRange.m6146getStartimpl(retransformed$iv)) {
                ...
                if (proposedCursor == TextRange.m6141getEndimpl(retransformed$iv)) {
                    state.setSelectionWedgeAffinity(new SelectionWedgeAffinity(WedgeAffinity.End));
                    return proposedCursor;
                }
                state.setSelectionWedgeAffinity(new SelectionWedgeAffinity(WedgeAffinity.Start));
