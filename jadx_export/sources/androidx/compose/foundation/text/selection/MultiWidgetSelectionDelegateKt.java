...
public final class MultiWidgetSelectionDelegateKt {
    ...
    public static final void m1332appendSelectableInfoParwq6A(SelectionLayoutBuilder $this$appendSelectableInfo_u2dParwq6A, TextLayoutResult textLayoutResult, long localPosition, long previousHandlePosition, long selectableId) {
        long j;
        ...
        Rect bounds = new Rect(0.0f, 0.0f, IntSize.m6867getWidthimpl(textLayoutResult.getSize()), IntSize.m6866getHeightimpl(textLayoutResult.getSize()));
        Direction currentXDirection3 = m1334getXDirection3MmeM6k(localPosition, bounds);
        Direction currentYDirection = m1335getYDirection3MmeM6k(localPosition, bounds);
        ...
        if (isSelected(startYHandleDirection, endYHandleDirection)) {
            ...
            if ($this$appendSelectableInfo_u2dParwq6A.getIsStartHandle()) {
                int rawStartHandleOffset3 = m1333getOffsetForPosition3MmeM6k(localPosition, textLayoutResult);
                ...
                startXHandleDirection2 = startXHandleDirection;
                int rawEndHandleOffset = m1333getOffsetForPosition3MmeM6k(localPosition, textLayoutResult);
            ...
            int rawPreviousHandleOffset = OffsetKt.m3966isUnspecifiedk4lQ0M(previousHandlePosition) ? -1 : m1333getOffsetForPosition3MmeM6k(previousHandlePosition, textLayoutResult);
    ...
    private static final Direction m1334getXDirection3MmeM6k(long position, Rect bounds) {
        return Offset.m3945getXimpl(position) < bounds.getLeft() ? Direction.BEFORE : Offset.m3945getXimpl(position) > bounds.getRight() ? Direction.AFTER : Direction.ON;
    }

    /* renamed from: getYDirection-3MmeM6k, reason: not valid java name */
    private static final Direction m1335getYDirection3MmeM6k(long position, Rect bounds) {
        return Offset.m3946getYimpl(position) < bounds.getTop() ? Direction.BEFORE : Offset.m3946getYimpl(position) > bounds.getBottom() ? Direction.AFTER : Direction.ON;
    }
    ...
    private static final int m1333getOffsetForPosition3MmeM6k(long position, TextLayoutResult textLayoutResult) {
        if (Offset.m3946getYimpl(position) <= 0.0f) {
            return 0;
        }
        return Offset.m3946getYimpl(position) >= textLayoutResult.getMultiParagraph().getHeight() ? textLayoutResult.getLayoutInput().getText().length() : textLayoutResult.m6118getOffsetForPositionk4lQ0M(position);
    }
}
