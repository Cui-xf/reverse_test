...
public abstract class AbstractClickableNode extends DelegatingNode implements PointerInputModifierNode, KeyInputModifierNode, FocusEventModifierNode, SemanticsModifierNode, TraversableNode {
    ...
    private AbstractClickableNode(MutableInteractionSource interactionSource, IndicationNodeFactory indicationNodeFactory, boolean enabled, String onClickLabel, Role role, Function0<Unit> function0) {
        ...
        this.centerOffset = Offset.INSTANCE.m3961getZeroF1C5BW0();
    ...
    protected final void m205updateCommonQzZPfjk(MutableInteractionSource interactionSource, IndicationNodeFactory indicationNodeFactory, boolean enabled, String onClickLabel, Role role, Function0<Unit> onClick) {
    ...
    public final void mo203onPointerEventH0pRuoY(PointerEvent pointerEvent, PointerEventPass pass, long bounds) {
        long $this$toOffset_u2d_u2dgyyYBs$iv = IntSizeKt.m6873getCenterozmzZPI(bounds);
        this.centerOffset = OffsetKt.Offset(IntOffset.m6825getXimpl($this$toOffset_u2d_u2dgyyYBs$iv), IntOffset.m6826getYimpl($this$toOffset_u2d_u2dgyyYBs$iv));
        initializeIndicationAndInteractionSourceIfNeeded();
        if (this.enabled && pass == PointerEventPass.Main) {
            int type = pointerEvent.getType();
            if (PointerEventType.m5353equalsimpl0(type, PointerEventType.INSTANCE.m5357getEnter7fucELk())) {
            ...
            } else if (PointerEventType.m5353equalsimpl0(type, PointerEventType.INSTANCE.m5358getExit7fucELk())) {
    ...
    public final boolean mo202onKeyEventZmokQxo(KeyEvent event) {
        initializeIndicationAndInteractionSourceIfNeeded();
        if (this.enabled && Clickable_androidKt.m277isPressZmokQxo(event)) {
            if (!this.currentKeyPressInteractions.containsKey(Key.m4946boximpl(KeyEvent_androidKt.m5257getKeyZmokQxo(event)))) {
                PressInteraction.Press press = new PressInteraction.Press(this.centerOffset, null);
                this.currentKeyPressInteractions.put(Key.m4946boximpl(KeyEvent_androidKt.m5257getKeyZmokQxo(event)), press);
                if (this.interactionSource == null) {
                    return true;
                }
        ...
        if (this.enabled && Clickable_androidKt.m275isClickZmokQxo(event)) {
            PressInteraction.Press it = this.currentKeyPressInteractions.remove(Key.m4946boximpl(KeyEvent_androidKt.m5257getKeyZmokQxo(event)));
    ...
    public final boolean mo204onPreKeyEventZmokQxo(KeyEvent event) {
        return false;
    }
    ...
    public final void applySemantics(SemanticsPropertyReceiver $this$applySemantics) {
        if (this.role != null) {
            ...
            SemanticsPropertiesKt.m5975setRolekuIjeqM($this$applySemantics, role.getValue());
        }
    ...
    protected final Object m201handlePressInteractiond4ec7I(PressGestureScope $this$handlePressInteraction_u2dd_u2d4ec7I, long offset, Continuation<? super Unit> continuation) {
