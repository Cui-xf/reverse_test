...
public final class VectorizedKeyframesSpec<V extends AnimationVector> implements VectorizedDurationBasedAnimationSpec<V> {
    ...
    public VectorizedKeyframesSpec(Map<Integer, ? extends Pair<? extends V, ? extends Easing>> map, int durationMillis, int delayMillis) {
        ...
        for (Map.Entry element$iv : map.entrySet()) {
            ...
            timeToInfoMap.set(time, new VectorizedKeyframeSpecElementInfo(valueEasing.getFirst(), valueEasing.getSecond(), ArcMode.INSTANCE.m146getArcLinear9TMq4(), null));
        }
        this(mutableIntList, timeToInfoMap, i, delayMillis, EasingKt.getLinearEasing(), ArcMode.INSTANCE.m146getArcLinear9TMq4(), null);
    }
    ...
    private final void init(V initialValue, V targetValue, V initialVelocity) {
        ...
        if (this.valueVector == null) {
            ...
            for (int i2 = 0; i2 < size2; i2++) {
                ...
                int iM200getArcMode9TMq4 = vectorizedKeyframeSpecElementInfo != null ? vectorizedKeyframeSpecElementInfo.m200getArcMode9TMq4() : this.initialArcMode;
                if (!ArcMode.m140equalsimpl0(iM200getArcMode9TMq4, ArcMode.INSTANCE.m146getArcLinear9TMq4())) {
                    z = true;
                }
                iArr[i2] = iM200getArcMode9TMq4;
            }
            this.modes = iArr;
        }
        if (!z) {
    ...
    public V getValueFromNanos(long playTimeNanos, V initialValue, V targetValue, V initialVelocity) {
        ...
        int iClampPlayTime = (int) VectorizedAnimationSpecKt.clampPlayTime(vectorizedKeyframesSpec, playTimeNanos / AnimationKt.MillisToNanos);
        if (vectorizedKeyframesSpec.keyframes.contains(iClampPlayTime)) {
            VectorizedKeyframeSpecElementInfo<V> vectorizedKeyframeSpecElementInfo = vectorizedKeyframesSpec.keyframes.get(iClampPlayTime);
            Intrinsics.checkNotNull(vectorizedKeyframeSpecElementInfo);
            return (V) vectorizedKeyframeSpecElementInfo.getVectorValue();
        }
        if (iClampPlayTime >= vectorizedKeyframesSpec.getDurationMillis()) {
            return targetValue;
        }
        if (iClampPlayTime <= 0) {
            return initialValue;
        ...
        if (vectorizedKeyframesSpec.arcSpline != null) {
            float easedTime = vectorizedKeyframesSpec.getEasedTime(iClampPlayTime);
        ...
        int iFindEntryForTimeMillis = vectorizedKeyframesSpec.findEntryForTimeMillis(iClampPlayTime);
        float easedTimeFromIndex = vectorizedKeyframesSpec.getEasedTimeFromIndex(iFindEntryForTimeMillis, iClampPlayTime, true);
        int i2 = vectorizedKeyframesSpec.timestamps.get(iFindEntryForTimeMillis);
        if (vectorizedKeyframesSpec.keyframes.contains(i2)) {
            VectorizedKeyframeSpecElementInfo<V> vectorizedKeyframeSpecElementInfo2 = vectorizedKeyframesSpec.keyframes.get(i2);
            Intrinsics.checkNotNull(vectorizedKeyframeSpecElementInfo2);
            vectorValue = vectorizedKeyframeSpecElementInfo2.getVectorValue();
        } else {
            vectorValue = initialValue;
        }
        int i3 = vectorizedKeyframesSpec.timestamps.get(iFindEntryForTimeMillis + 1);
        if (vectorizedKeyframesSpec.keyframes.contains(i3)) {
            VectorizedKeyframeSpecElementInfo<V> vectorizedKeyframeSpecElementInfo3 = vectorizedKeyframesSpec.keyframes.get(i3);
            Intrinsics.checkNotNull(vectorizedKeyframeSpecElementInfo3);
        ...
        while (true) {
            ...
            v.set$animation_core_release(i4, VectorConvertersKt.lerp(vectorValue.get$animation_core_release(i4), vectorValue2.get$animation_core_release(i4), easedTimeFromIndex));
            i4++;
