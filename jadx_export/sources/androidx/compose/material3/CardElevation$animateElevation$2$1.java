    ...
    public final Object invokeSuspend(Object $result) {
        ...
        switch (this.label) {
            case 0:
                ...
                if (!Dp.m6698equalsimpl0(this.$animatable.getTargetValue().m6707unboximpl(), this.$target)) {
                    boolean z = this.$enabled;
                    Animatable<Dp, AnimationVector1D> animatable = this.$animatable;
                    if (!z) {
                        this.label = 1;
                        if (animatable.snapTo(Dp.m6691boximpl(this.$target), this) == coroutine_suspended) {
                            return coroutine_suspended;
                        }
                        cardElevation$animateElevation$2$12 = this;
                    } else {
                        float fM6707unboximpl = animatable.getTargetValue().m6707unboximpl();
                        DragInteraction.Start start = null;
                        if (Dp.m6698equalsimpl0(fM6707unboximpl, this.this$0.pressedElevation)) {
                            start = new PressInteraction.Press(Offset.INSTANCE.m3961getZeroF1C5BW0(), null);
                        } else if (Dp.m6698equalsimpl0(fM6707unboximpl, this.this$0.hoveredElevation)) {
                            start = new HoverInteraction.Enter();
                        } else if (Dp.m6698equalsimpl0(fM6707unboximpl, this.this$0.focusedElevation)) {
                            start = new FocusInteraction.Focus();
                        } else if (Dp.m6698equalsimpl0(fM6707unboximpl, this.this$0.draggedElevation)) {
                            start = new DragInteraction.Start();
                        }
                        this.label = 2;
                        if (ElevationKt.m2918animateElevationrAjV9yQ(this.$animatable, this.$target, start, this.$interaction, this) == coroutine_suspended) {
                            return coroutine_suspended;
                        }
